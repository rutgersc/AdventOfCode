...
main = defaultMain $ testCase "Day2b tests" $ do
    ...
    assertEqual ""    Nothing $ findDivisableInListRec 5 [9,2,8] Nothing 
    assertEqual ""    Nothing $ findDivisableInListRec 9 [2,8] Nothing
    assertEqual ""   (Just 4) $ findDivisableInListRec 2 [8] Nothing
...
findDivisable :: [Int] -> Int
findDivisable = findDivisableRec Nothing

findDivisableRec :: Maybe Int -> [Int] -> Int
findDivisableRec (Just v) _  = v
findDivisableRec Nothing (x:xs) = findDivisableRec (findDivisableInListRec x xs Nothing) xs
findDivisableRec Nothing _ = 0

findDivisableInListRec :: Int ->  [Int] -> Maybe Int -> Maybe Int
findDivisableInListRec curr _ (Just v)  = Just v
findDivisableInListRec curr (x:xs) Nothing = findDivisableInListRec curr xs (tryEvenlyDivide curr x)  
findDivisableInListRec curr _ Nothing = Nothing

tryEvenlyDivide :: Int -> Int -> Maybe Int
tryEvenlyDivide 0 _ = Nothing
tryEvenlyDivide _ 0 = Nothing
tryEvenlyDivide a b = selectResult (quotRem a b) (quotRem b a)
